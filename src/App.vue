...
// reactive함수: 객체 데이터의 반응성을 주입하여 값이 변경되면 리렌더링이 되어 화면이 갱신됨
const data = reactive({
  newItem: '',
  items: [],
});
...
// 할 일 추가
// 할 일 객체데이터는 고유 id추가하여 데이터를 구분할 수 있도록 해야함
function addItem() {
  data.items.push({
    id: data.items.length,
    text: data.newItem,
    completed: false, // 처음엔 완료가 아니니 false로 초기화 해둠
  });
  // 할 일 추가 뒤 입력필드와 연결된 데이터를 빈문자로 초기화
  data.newItem = '';
}
...
<template>
  <main class="app">
    ...
    <div class="todo_count">
      완료된 할 일: {{ isComplete }} / 할 일: {{ isComplete }}
    </div>
    <div class="todo_add">
...
.remove_btn {
  ...
  padding: 0 10px;
  background: none;
  border: 1px solid var(--point1-color1);
